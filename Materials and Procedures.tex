...
\end{equation}
The outer boundary of the involute, $r$, grows in a near linear fashion with the number of revolutions, while the path (or arc) length, $L$, is given by $L=\frac{R}{2}\cdot\alpha^2$, growing quadratically with the number of revolutions. The time to complete a survey, $t$, at a swimming speed $s$ is then $t = \frac{L}{s} = \frac{R}{2s}\cdot\alpha^2$
